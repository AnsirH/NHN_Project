...
namespace OutGame.UI.Deployment
{
    ...
    /// 그렇게 미리 존재하는 슬롯들을 찾아 이벤트를 전달하고 전투력 라벨을 갱신하는 역할만 한다.
    ...
    {
        [SerializeField] private RectTransform slotArea; // 미리 구워진 DeploySlotView들의 부모
        ...
        private void Awake() => DiscoverSlotsIfNeeded();

        /// <summary>슬롯 발견(딕셔너리 채우기 + 이벤트 전달 배선)은 Awake()에만 기대지 않고 여기서
        ...
        /// slotsDiscovered 플래그로 중복 실행(이벤트 이중 구독)은 막는다 — Awake는 원래 한 번만 돌지만,
        ...
        {
            if (slotsDiscovered) return;
            slotsDiscovered = true;

            if (slotArea == null || powerLabel == null || fieldConfig == null)
                throw new InvalidOperationException("FormationGridView의 구조 참조가 배선되지 않았습니다.");

            foreach (DeploySlotView slot in slotArea.GetComponentsInChildren<DeploySlotView>(includeInactive: true))
            {
            ...
            if (slotViewsById.Count != expectedCount)
                throw new InvalidOperationException(
                    $"FormationGridPanel에 구워진 슬롯 수({slotViewsById.Count})가 BattleFieldConfig" +
