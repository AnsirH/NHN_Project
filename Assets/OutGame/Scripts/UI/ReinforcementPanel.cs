...
namespace OutGame.UI
{
    ...
    /// 영구 증원이라 용어를 맞췄다. 2026-08-08: 클래스명(RestPanel)도 그때 남겨뒀던 "휴식" 어휘를
    /// 마저 정리해 ReinforcementPanel로 개명 — Docs/OutGame/화면 명칭 정리.md): 보유 군대 중 1개
    /// 선택 → 병사 수 영구 +20% 증원.
    ...
    public class ReinforcementPanel : MonoBehaviour
    {
        ...
        private void Awake()
        {
            if (allyFormationView == null || resultPopupRoot == null || resultText == null || continueButton == null)
                throw new InvalidOperationException("ReinforcementPanel 프리팹의 필드가 배선되지 않았습니다.");
        ...
        private void OnArmySelected(string armyInstanceId)
        {
            ...
            {
                Debug.LogWarning($"[ReinforcementPanel] 병과에 대한 ArmyDefinition을 찾을 수 없어 증원할 수 없습니다: {armyInstanceId}");
                return;
            }

            var data = def.ToData();
            int added = ReinforcementService.Reinforce(army, data);
