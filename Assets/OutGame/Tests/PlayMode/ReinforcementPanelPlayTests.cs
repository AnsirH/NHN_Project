...
{
    /// <summary>
    /// ReinforcementPanel 프리팹 스모크 테스트 — 프리팹은 SceneSetupM4UI.Run()으로 생성돼 있어야 한다.
    /// 증원 대상 선택은 배치 화면과 동일한 진영 그리드(AllyFormationView)를 재사용한다(2026-07-26
    /// 사용자 요청) — 카드를 누르면 그 즉시 증원된다(확인 절차 없음).
    /// </summary>
    public class ReinforcementPanelPlayTests
    {
        private GameObject canvasGo;
        private GameObject eventSystemGo;
        private ReinforcementPanel panel;
        ...
        public void SetUp()
        {
            ...
            GameObject prefab = Resources.Load<GameObject>("OutGame/Panels/ReinforcementPanel");
            Assert.IsNotNull(prefab, "ReinforcementPanel 프리팹 없음 — SceneSetupM4UI.Run() 실행 필요");
            panel = Object.Instantiate(prefab, canvasGo.transform).GetComponent<ReinforcementPanel>();
